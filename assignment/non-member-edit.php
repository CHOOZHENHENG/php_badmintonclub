...
if (!empty($_POST)){
    ...
    $event = addslashes($events);
      ...
      if (empty($error)){
          ...
          $q = "UPDATE non_member_eb SET Name = '$name', Gender = '$gender', Mobile_phone = '$phone', Email_address = '$email', Event = '$event'
...
$q = "SELECT * FROM non_member_eb WHERE stud_id = '$student_id'";
$r = mysqli_query($dbc, $q); 
...
<html>
    ...
    <title>Edit Non-member Record</title>
    ...
    <body>
        <form action="non-member-edit.php" method="POST">
            <fieldset>
                <h1 style="color: blue">Edit Non-member Record</h1>
                <hr>
                 <p style="color: brown">
                    ID:
                    <?php echo $student_id; ?>
            ...
            <input type="button" name="Cancel" value="Back to Member List" onclick="location='admin-non-member-list.php'" />
                </div>
            </fieldset>
        </form>
