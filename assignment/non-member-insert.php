...
if (!empty($_POST)){
    ...
    $non_member_id = strtoupper(trim($_POST['non_member_id']));
    ...
    if(isset($_POST['gender'])){
      ...
      $error = array();
      if($non_member_id == null){
          $error['non_member_id'] = 'Please enter the <strong>ID</strong>.';
      }
      if($non_member_name == null){
          $error['non_member_name'] = 'Please enter the <strong>Name</strong>.';
      }
      if($gender == null){
      ...
      if (empty($error)){
          ...
          $q = "INSERT into Non_Member (Non_MemberID, Non_MemberName, Gender, Phone, Email, Member, Event)VALUES ('$non_member_id', '$non_member_name', '$gender', '$phone', '$email', '$member', '$event')";
          $r = @mysqli_query($dbc, $q);
          
          if($r){
              echo '<div style="background-color:#6CCA9B">'.
                      '<h1 tyle="text-align:center;font-size: 2em;">Thank You!</h1>'
                      . '<p class="error" style="text-align:center;font-size: 1em;color:black;">The non-member has been registered successfully.</p>'.
                      '</div>';
          }else{
              echo '<div style="background-color:#6CCA9B">'.
                      '<h1 tyle="text-align:center;font-size: 2em;">Stystem Error!</h1>'
                      . '<p class="error" style="text-align:center;font-size: 1em;color:black;">The non-member cannot be registered due to a system error.</p>'.
                      '</div>';
          }
          mysqli_close($dbc);
          }
...
<html>
    ...
    <title>Insert Non-Member Record</title>
    ...
    <body>
        <form action="non-member-insert.php" method="POST">
            <fieldset>
                <h1 style="color: blue">Insert Non-Member Record</h1>
                <hr>
                 <p style="color: brown">
                    ID:
                    <input type="text" name="non_member_id" value="<?php echo $non_member_id; ?>" maxlength="10" /></td>
                </p>
                <p style="color: brown">
                    Name:
                    <input style="margin-left:51px;" type="text" name="non_member_name" value="<?php echo $non_member_name; ?>"/>
                </p>
                <p style="color: brown">
                    Gender: 
            ...
            <input type="button" name="Cancel" value="Back to Non-Member List" onclick="location='non-member.php'" />
                </div>
            </fieldset>
        </form>
